...
# BEAUTIFUL CSS (keeping your premium design)
app.index_string = '''
<!DOCTYPE html>
...
.ultra-header{background:linear-gradient(135deg,#667eea,#764ba2,#f093fb);padding:3rem;border-radius:25px;box-shadow:0 20px 80px rgba(102,126,234,0.6);margin-bottom:3rem;position:relative;overflow:hidden;animation:headerPulse 3s ease-in-out infinite}
...
.metric-card{background:rgba(255,255,255,0.08);backdrop-filter:blur(30px);border:1px solid rgba(255,255,255,0.15);border-radius:20px;padding:2rem;text-align:center;transition:all 0.4s;cursor:pointer;position:relative;overflow:hidden;animation:fadeInUp 0.6s ease-out}
...
app.layout = html.Div([
    ...
    html.Div([html.H2("📥 Upload", style={"color": "white", "textAlign": "center", "marginBottom": "2rem", "fontFamily": "Orbitron"}),
              dcc.Upload(id="upload", children=html.Div([html.I(className="fas fa-cloud-upload-alt", style={"fontSize": "4rem", "color": "#667eea"}),
                                                         html.H3("Drop Survey File", style={"color": "white", "marginTop": "1rem"})]),
                        className="upload-zone")],
             className="analysis-card"),
    
    html.Div(id="everything"),
    dcc.Store(id='data'),
], style={"padding": "2rem", "maxWidth": "1600px", "margin": "0 auto", "minHeight": "100vh"})

@app.callback([Output("everything", "children"), Output("m1", "children"), Output("m2", "children"), Output("m3", "children"), Output("data", "data")],
...
def process_all(c, f):
    ...
    try:
        ...
        types = [html.Li(f"{col}: {detect_question_type(df[col], col).upper()}", style={"padding": "0.5rem"}) for col in df.columns]
        scales = [html.Li(f"{col}: {detect_scale_orientation(df[col].dropna(), col)}", style={"padding": "0.5rem"}) 
        ...
        industry = 'restaurant' if sum(all_text.count(w) for w in ['food', 'ice', 'cream', 'flavor']) > 50 else 'general'
        
        pos = sum(1 for t in df[text_col].head(20) if any(w in str(t).lower() for w in ['good', 'great', 'excellent', 'amazing', 'love']))
        sent = (pos / 20) * 100
        ...
        fig1 = go.Figure(data=[go.Pie(labels=['Positive', 'Neutral', 'Negative'], values=[pos, 20-pos-2, 2],
                                       marker=dict(colors=['#10b981', '#f59e0b', '#ef4444']), hole=0.5)])
        fig1.update_layout(title="💭 Sentiment", plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color='white'), height=450)
        
        fig2 = go.Figure(data=[go.Bar(x=['Your Performance', 'Industry Average'], y=[sent, 60],
                                      marker=dict(color=['#10b981', '#95a5a6']), text=[f'{sent:.0f}%', '60%'], textposition='outside')])
        fig2.update_layout(title="📊 Benchmark", plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color='white'), height=450)
        
        if comps:
            fig3 = go.Figure(data=[go.Bar(y=list(comps.keys()), x=list(comps.values()), orientation='h',
                                          marker=dict(color=['#ffd700', '#c0c0c0'][:len(comps)]))])
            fig3.update_layout(title="🏆 Competitor Mentions", plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color='white'), height=450)
            comp_chart = dcc.Graph(figure=fig3, config={'displayModeBar': False})
        else:
            comp_chart = html.P("No competitors found", style={"color": "rgba(255,255,255,0.6)", "textAlign": "center", "fontSize": "1.2rem"})
        ...
        return html.Div([
            ...
            html.Div([
                html.Div([html.H3("🔍 Question Type Detection", style={"marginBottom": "1.5rem", "fontFamily": "Orbitron"}), html.Ul(types, style={"listStyle": "none"})], className="analysis-card"),
                html.Div([html.H3("📏 Scale Orientation", style={"marginBottom": "1.5rem", "fontFamily": "Orbitron"}), html.Ul(scales, style={"listStyle": "none"})], className="analysis-card")
            ...
            html.Div([
                html.H3("📤 Export & Download", style={"textAlign": "center", "marginBottom": "2rem", "fontFamily": "Orbitron"}),
                html.P("Analysis complete! Ready for research paper and presentations.", style={"textAlign": "center", "color": "rgba(255,255,255,0.7)", "fontSize": "1.1rem"})
            ], className="analysis-card", style={"marginTop": "3rem"})
            
        ]), "1", industry.upper()[:4], f"{sent:.0f}%", df.to_dict('records')
        
    except Exception as e:
        return html.Div(f"Error: {str(e)}", style={"color": "#ef4444"}), "0", "---", "0%", None
